...
class PaklaringController extends Controller
{
    ...
    public function store(StorePaklaringRequest $request, PaklaringPdfService $pdfService): RedirectResponse
    {
        $this->authorize('create', Paklaring::class);
    ...
    public function batchStore(Request $request, PaklaringPdfService $pdfService): RedirectResponse
    {
        $this->authorize('create', Paklaring::class);
        ...
        $validated = $request->validate([
            ...
            // Filled in manually by the user, one per selected employee —
            // not generated by the system.
            'no_surats' => ['required', 'array'],
            'no_surats.*' => ['required', 'string', 'max:255', 'distinct', 'unique:paklarings,no_surat'],
        ...
        $ready = $employees->filter(fn (Employee $employee) => $employee->is_complete_for_paklaring
            && filled($validated['no_surats'][$employee->id] ?? null));
        $skipped = $employees->diff($ready);

        $created = DB::transaction(function () use ($ready, $validated, $user, $pdfService) {
            $paklarings = collect();
            ...
            foreach ($ready as $employee) {
                ...
                $data = [
                    'site_id' => $site->id,
                    'no_surat' => $validated['no_surats'][$employee->id],
                    'nrpp' => $employee->nrpp,
                    'nama' => $employee->nama,
        ...
        if ($skipped->isNotEmpty()) {
            $message .= ' '.$skipped->count().' dilewati karena data karyawan belum lengkap atau nomor surat belum diisi: '
    ...
    public function destroy(Paklaring $paklaring): RedirectResponse
    {
    ...
    public function bulkDestroy(Request $request): RedirectResponse
    {
        ...
        $paklarings->each(function (Paklaring $paklaring) {
            if ($paklaring->file_path) {
                Storage::disk('local')->delete($paklaring->file_path);
            }
        });
